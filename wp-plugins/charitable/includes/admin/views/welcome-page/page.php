...
<div class="wrap about-wrap charitable-wrap">
	...
	<div class="badge">
		<a href="<?php echo charitable_ga_url(
			'https://www.wpcharitable.com',
			urlencode( 'Welcome Page Icon'),
			urlencode( 'Icon' ) ) ?>" target="_blank"><i class="charitable-icon charitable-icon-charitable"></i></a>
	</div>
	<div class="intro">
		<?php
		...
		$view_pricing_url = charitable_ga_url(
			...
			urlencode( 'Welcome Page Sidebar Link'),
			urlencode( 'View Pricing' )
		);
	...
	<div class="column-left">
		<div class="column-inside">
			...
			<?php if ( current_user_can( 'manage_charitable_settings' ) ) : ?>
				...
				<?php
					...
					$doc_url = charitable_ga_url(
						...
						urlencode( 'Dashboard Intro Copy Doc'),
						urlencode( 'our documentation' )
					);

					$support_url = charitable_ga_url(
						'https://www.wpcharitable.com/support/',
						urlencode( 'Dashboard Intro Copy Support'),
						urlencode( 'our support' )
					);

				?>
				<p style="margin-bottom: 0;"><?php
					printf(
						__( 'Need a hand with anything? You might find the answer in <a target="_blank" href="%s">our documentation</a>, or you can always get in touch with us via <a target="_blank" href="%s">our support page</a>.', 'charitable' ),
						$doc_url,
						$support_url
					); ?>
				</p>
			...
			<?php if ( strpos( $locale, 'en' ) !== 0 ) : ?>
				...
				<p><?php printf( __( 'You can help us translate Charitable into %s by <a target="_blank" href="https://translate.wordpress.org/projects/wp-plugins/charitable">contributing to the translation project</a>.', 'charitable' ),
					$language
				); ?></p>
				<hr />
			<?php endif ?>
			...
			<?php
				...
				$download_url = charitable_ga_url(
					...
					urlencode( 'Dashboard Reach Download Button'),
					urlencode('Download It Free')
				);

				$view_demo = charitable_ga_url(
					'https://demo.wpcharitable.com/reach/',
					urlencode( 'Dashboard Demo Reach Button'),
					urlencode( 'View Demo')
				);

			?>
			<p><a href="<?php echo $download_url; ?>" class="button-primary" style="margin-right: 8px;" target="_blank"><?php _e( 'Download it free', 'charitable' ); ?></a><a href="<?php echo $view_demo; ?>" class="button-secondary" target="_blank"><?php _e( 'View demo', 'charitable' ); ?></a></p>
		</div>
	</div>
	<div class="column-right">
		<div class="bundle-promo">
			...
			<p style="text-align: center;"><a href="<?php echo $view_pricing_url; ?>" class="button-primary" target="_blank" rel="noopener"><?php _e( 'Get Started with Charitable Pro', 'charitable' ); ?></a></p>

			<small><em><?php _e( 'Bonus: Charitable Lite users save $100 or more off the regular price, automatically applied at checkout!', 'charitable' ); ?></em></small>
		</div>
		<div class="column-inside">
			...
			<ul class="extensions">
				<?php foreach ( $extensions as $extension => $description ) :
					...
					$extension_url = charitable_ga_url(
						...
						urlencode( 'Recommended Extensions'),
						$extension
					);

...
if ( ! class_exists( 'Charitable_Campaign_Processor' ) ) :
	...
	class Charitable_Campaign_Processor {
		...
		public function save() {
			...
			if ( ! $this->campaign_id || is_wp_error( $this->campaign_id ) ) {
				return 0;
			}
		 ...
		 * @since  1.5.9
		 *
		 * @return int|WP_Error The donation ID if succesful. The value 0 or WP_Error on failure.
		 */
		 ...
		 *
		 * @since  1.5.9
		 * @since  1.6.54 Return an array, instead of true.
		 *
		 * @return array The meta data saved, in key=>value format.
		 */
		public function save_meta() {
			...
			array_walk( $meta, array( $this, 'save_meta_field' ) );
		 ...
		 *
		 * @since  1.5.9
		 * @since  1.6.54 Return an array, instead of void.
		 *
		 * @return array Taxonomy data that was saved.
		 */
		...
		/**
		 * Sanitize the suggested donations default
		 *
		 * @since  1.5.9
		 *
		 * @param  arrat $value Current value of suggested_donations.
		 * @return array
		 */
		protected function sanitize_suggested_donations_default( $value ) {
			if ( $this->should_skip_sanitizer_for_key( 'suggested_donations_default', array( 'Charitable_Campaign', 'sanitize_campaign_suggested_donations' ) ) ) {
				return $value;
			}

			return Charitable_Campaign::sanitize_campaign_suggested_donations_default( $value );
		}

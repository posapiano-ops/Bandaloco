...
{
    add_meta_box('event_tickets',esc_html__('Registrant Tickets','borntogive-core'),'borntogive_event_tickets_output','event_registrants','normal','core');
}
/**
    ...
    <div id="field_wrap">
        ...
        <?php
				if(!empty($tickets_type))
				{
					...
					{
						echo '<div><label>'.esc_attr__('Ticket Type: ', 'borntogive-core').esc_attr($key).'</label></div>';
						echo '<div><label>'.esc_attr__('Number of Tickets: ', 'borntogive-core').esc_attr($value).'</label></div>';
						echo '<br/>';
					}
				}
				?>

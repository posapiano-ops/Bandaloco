...
{
    add_meta_box('event_schedule',esc_html__('Event Tickets Type','borntogive-core'),'borntogive_event_feilds_output','event','normal','core');
}
/**
...
<div id="field_group">
    ...
    if ( !empty( $tickets_type) ) 
    {
        ...
        <?php if($row_head==1)
				{ 
					echo '<label class="heading1">'.esc_attr__('Ticket Title', 'borntogive-core').'</label>';
					echo '<label class="heading2">'.esc_attr__('Total Tickets', 'borntogive-core').'</label>';
					echo '<label class="heading3">'.esc_attr__('Booked Tickets', 'borntogive-core').'</label>';
					echo '<label class="heading4">'.esc_attr__('Ticket Cost', 'borntogive-core').'</label>';
					echo '<br/>';
				}
					?>
        <div class="field_row">
        <div class="field_left">
              ...
              <input type="text" class="meta_feat_title" name="featured[end_time][]" value="<?php echo esc_attr($ticket_total); ?>" placeholder="No of Tickets" style="width:15%">
                ...
                <input class="button" type="button" value="<?php esc_html_e('Remove','borntogive-core'); ?>" onclick="remove_field(this)" /> 
        </div>
          <div class="clear" /></div> 
        </div>
        <?php $row_head++;
    ...
    <div id="add_field_row">
      <input class="button" type="button" value="<?php esc_html_e('Add Ticket','borntogive-core'); ?>" onclick="add_field_row();" />
      <p><?php echo esc_attr_e('Booked Ticket field will update automatically.', 'borntogive-core'); ?></p>
      <p><?php echo esc_attr_e('Do not add currency in price field, currency should selected from Theme Options.', 'borntogive-core'); ?></p>
      <p><?php echo esc_attr_e('Do not add Ticket field to use free registration.', 'borntogive-core'); ?></p>
    </div>
</div>
  <?php
}
...
function add_admin_scripts_event( $hook ) {
    ...
    if ( $hook == 'post-new.php' || $hook == 'post.php' ) {
        if ( 'event' === $post->post_type ) {     
            wp_enqueue_script(  'event_clone_tickets_js', BORNTOGIVE_THEME_PATH.'/assets/js/clone_fields.js' );
			wp_enqueue_style(  'event_clone_tickets_style', BORNTOGIVE_THEME_PATH.'/assets/css/clone_fields.css' );
        }
    }
}

...
function vestige_image_category_tax_custom_fields($tag) {
       ...
       <table class="form-table">
           <tbody><tr class="form-field form-required">
                   <th scope="row"><label for="image"><?php esc_html_e('Term Banner Image', 'borntogive-core') ?></label></th>
                   <td><?php
                       echo '<div><img id ="banner_image_preview" src ="' . $term_meta . '" width ="150px"/></div>';
                       echo '<input id="upload_term_button" type="button" class="button button-primary" value="'.esc_html__('Upload Image', 'borntogive-core').'" /> ';
                      if(isset($_REQUEST['tag_ID'])){
                       ...
                       ?>
                   <p class="description"><?php esc_html_e('Upload an image for the taxonomy .', 'borntogive-core'); ?></p></td>

...
if ( ! class_exists( 'Charitable_Polylang_Compat' ) ) :
	...
	class Charitable_Polylang_Compat {
		...
		public function __construct() {
			...
			/* Disable pre_get_posts filter added by Polylang */
			add_action( 'charitable_donation_stats_before', array( $this, 'disable_polylang_pre_get_posts_filter' ) );
			add_action( 'charitable_donation_stats_after', array( $this, 'reenable_polylang_pre_get_posts_filter' ) );
		...
		/**
		 * Disable the Polylang auto translate pre_get_posts filter.
		 *
		 * @since  1.6.57
		 *
		 * @return void
		 */
		public function disable_polylang_pre_get_posts_filter() {
			if ( method_exists( \PLL(), 'auto_translate' ) ) {
				remove_action( 'pre_get_posts', array( \PLL()->auto_translate(), 'pre_get_posts' ) );
			}
		}

		/**
		 * Re-enable the Polylang auto translate pre_get_posts filter.
		 *
		 * @since  1.6.57
		 *
		 * @return void
		 */
		public function reenable_polylang_pre_get_posts_filter() {
			if ( method_exists( \PLL(), 'auto_translate' ) ) {
				add_action( 'pre_get_posts', array( \PLL()->auto_translate(), 'pre_get_posts' ) );
			}
		}
	}

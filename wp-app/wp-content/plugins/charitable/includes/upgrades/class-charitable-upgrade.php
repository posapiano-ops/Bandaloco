	...
	class Charitable_Upgrade {
		...
		protected function __construct( $db_version = '', $edge_version = '' ) {
			...
			$this->upgrade_actions = array(
				...
				'flush_permalinks_1621'                   => array(
					'version'  => '1.6.21',
					'message'  => '',
					'prompt'   => false,
					'callback' => array( $this, 'flush_permalinks' ),
				),
				'remove_campaign_manager_unfiltered_html_cap' => array(
					'version'  => '1.6.51',
					'message'  => '',
					'prompt'   => false,
					'callback' => array( $this, 'remove_campaign_manager_unfiltered_html_cap' ),
				),
			);
		}
...
<?php
		...
		/**
		 * Update the unfiltered_html cap for Campaign Managers.
		 *
		 * @since  1.6.51
		 *
		 * @return true
		 */
		public function remove_campaign_manager_unfiltered_html_cap() {
			global $wp_roles;
			...
			if ( is_object( $wp_roles ) ) {
				$wp_roles->remove_cap( 'campaign_manager', 'unfiltered_html' );
			}
